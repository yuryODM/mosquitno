...
class MicArray(object):
    ...
    def get_direction(self, buf):
        ...
        if self.channels == 8:
            ...
            MIC_GROUP_N = 2
            MIC_GROUP = [[1, 3], [2, 4]]  # channel indices in your buffer

            tau = [0] * MIC_GROUP_N
            theta = [0] * MIC_GROUP_N

            for i, v in enumerate(MIC_GROUP):
                # buf[v[0]::5] -> start at channel index, step by total channels (5 including ASR channel)
                tau[i], _ = gcc_phat(buf[v[0]::5], buf[v[1]::5], fs=self.sample_rate, max_tau=MAX_TDOA_4, interp=1)
                theta[i] = math.asin(tau[i] / MAX_TDOA_4) * 180 / math.pi

            # Combine the two theta estimates into a single azimuth
            ...
            # Optional calibration offset to match your physical array orientation
